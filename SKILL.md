---
name: order-coffee
description: Order Luckin (瑞幸) coffee via the my-coffee MCP — manage favorite stores, search products (exact then tokenized fallback), place an order, auto-poll payment, and deliver a verified pickup QR + code. Use when the user wants to order coffee / 点咖啡 / 瑞幸 / luckin / 来一杯.
---
...
- Config stores **only** store preferences + coordinates. **Never** write tokens, phone
  numbers, or payment info anywhere. Login state belongs to the MCP client.
...
- If `init` reported `created=true`, tell the user the config path and that this is first-time
  setup; offer to register one or more favorite stores now (Phase 1 store search for each).
- If the runtime has a memory feature, store a one-line signpost only: config path + favorite
  store names. Never write deptId/coords to memory — JSON is the source of truth.

To read/write config: `resolve_config.js path` gives you the resolved file path; edit that JSON
file directly (keep `version:1`, keep it `chmod 600`). Never add token/phone/payment fields.
...
## Phase 2 — Store open + self-pickup check
...
## Phase 3 — Find the product (tiered)
...
## Phase 4 — Preview + confirm (four elements)
...
## Phase 5 — Create order + poll payment
...
3. **Auto-poll** (no need for the user to tell you they paid). Between checks, wait with a background sleep
   ...
   **15s, 30s, 45s, 60s (15s apart), then every 20s (80/100/120/140s), final check at 150s.**
   ...
   - **Paid** = `orderStatus` != initial **AND** `code` non-empty **AND** `code` != `INITIAL_CODE`
     → stop immediately, go to Phase 6.
...
## Recovery
If the user says "查一下我刚才的订单", query the latest order and resume: 待付款 → run one check (Phase 5.4)
and optionally re-poll; already paid → Phase 6.
...
adds it, offer the choice at Phase 4 and pass it through. Until then, don't assume it or smuggle it via `remark`.
